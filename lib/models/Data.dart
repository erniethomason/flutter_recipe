...
class Data {
  ...
  int activeRecipeIndex = -1;   // -1 : no active recipe ; not implemented, but can be used to reopen last active recipe when app starts
  ...
  Future<void> startup () async {
    ...
    final prefs = await SharedPreferences.getInstance();  // documentation shows using await
    ...
    if (rJson.length != 0) {
      ...
      for (Recipe loadR in loadRecipes.recipes) {
        Recipe recipe = new Recipe(loadR.name);
        for (Ingredient loadI in loadR.ingredients) {
          recipe.ingredients.add(new Ingredient(loadI.name, loadI.amount, loadI.unit));
        }
        r.recipes.add(recipe);
      }
    }
    activeRecipeIndex = prefs.getInt("defaultRecipeIndex") ?? -1;   // if doesn't exist, return -1
  }
